...
# creating a new list with only the required data

survival_list <- titanic %>% 
  select(class, survived) %>% 
  mutate(class = case_when(
    class 
    %in% c("3rd", "2nd", "1st") ~ "passenger",
    TRUE                        ~ "crew"))


# calculating totals

crew_total <- survival_list %>% 
  filter(class == "crew") %>% 
  summarise(n())

passenger_total <- survival_list %>% 
  filter(class == "passenger") %>% 
  summarise(n())

# Probabilities

prob_crew_dead <- survival_list %>% 
  filter(class == "crew", survived == "no") %>% 
  summarise(prob = n()/crew_total) %>% 
  pull()

prob_passenger_dead <- survival_list %>% 
  filter(class == "passenger", survived == "no") %>% 
  summarise(prob = n()/passenger_total) %>% 
  pull()


comparison <- c("prob_crew_dead" = prob_crew_dead, "prob_passenger_dead" = prob_passenger_dead)
...
rolldie(3) %>% 
  summarise(number_of_outcomes = n())
...
event_prbability <- total_equals_5_6_7/total_outcomes

event_prbability


# We assumed order does not matter and values can repeat
...
outcome_probaility <- 1/chair_arrangements
