...
null_distribution <- msleep %>% 
  ...
  hypothesise(null = "point", mu = 7) %>%        #Jabberwockies sleep for around 7 hours a night
  generate(reps = 10000, type = "bootstrap") %>% 
  calculate(stat = "mean") 

null_distribution %>% 
  visualise(bin = 30) +
  shade_p_value(obs_stat = observed_stats$sample_mean, direction = "both")



get_p_value <- null_distribution %>% 
  get_p_value(
    obs_stat = observed_stats$sample_mean, direction = "both")

get_p_value

# Taking a conventional value of a, i.e a = 0.05 
# Ho is 7 hours; i.e sample population mean sleep_total is equal to 7 hours.
# Ha is not equal to 7 hours; i.e sample population mean sleep_total is not equal to 7 hours.
# From the results we see that the p_value < a, as such we reject Ho and accept Ha.
...
null_distribution3 %>% 
  visualise(bin = 30) +
  shade_p_value(obs_stat = observed_stats3$stat, direction = "greater")  # greater as omnivores need to sleep longer than herbivores



get_p_value3 <- null_distribution3 %>% 
  get_p_value(obs_stat = observed_stats3$stat, direction = "greater")

get_p_value3
    
    
# Taking a conventional value of a, i.e a = 0.05 
# Ho µ total sleep for omni - µ total sleep herbi > 0 ; i.e omnivores sleep for significantly longer than herbivores
# Ha µ total sleep for omni - µ total sleep herbi = 0; is independent of the explanatory; i.e omnivores do not sleep for significantly longer than herbivores.
# From the results we see that the p_value > a, as such we reject Ho.
# But is it possible to say from the graph that they sleep the same? the total mean sleep seems to be equal 
...
domesticated_animals <- msleep %>% 
  mutate(is_domesticated = case_when(conservation == "domesticated" ~ "yes", 
                                     TRUE  ~  "no"), 
         .after = conservation)

domesticated_animals

observed_stats4 <- domesticated_animals %>% 
...
null_distribution4 <- domesticated_animals %>% 
  specify(response = is_domesticated, success = "yes") %>% 
...
null_distribution4 %>% 
  visualise() +
  shade_p_value(obs_stat = observed_stats4$stat, direction = "greater")

get_p_value4 <- null_distribution4 %>% 
  get_p_value(obs_stat = observed_stats4$stat, direction = "greater")

get_p_value4

# Taking a conventional value of a, i.e a = 0.05 
# Ho % of domesticated animals = 0.05; i.e sample population of domesticated animals is equal to 5%.
# Ha % of domesticated animals > 0.05; i.e sample population of domesticated animals is greater than 5%.
# From the results we see that the p_value < a, as such we accept Ho
