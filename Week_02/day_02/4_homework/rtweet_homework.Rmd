...
# not to sure by mean number of tweets

code_clan_tweets %>%
  select(retweet_count, source) %>% 
  group_by(source) %>% 
  summarise(mean = sum(retweet_count)/n())
...
code_clan_tweets$media_type[is.na(code_clan_tweets$media_type)] <-"text"
...
code_clan_tweets %>% 
  ...
  mutate(mean = sum(display_text_width)/ n()) %>% 
...
code_clan_info <- full_join(code_clan_tweets,code_clan_info, by = "tweet_id")
...
```{r}
codeclan_hastags <- code_clan_info %>%
